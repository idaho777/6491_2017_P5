...
class pt 
   ...
   @Override public boolean equals(Object o) { pt p = (pt) o; return p.x == x && p.y ==y && p.z == z; }
...
float angle2(vec V) {return(atan2(V.y,V.x)); };                                                       // angle between <1,0> and V (between -PI and PI)
float angle2(pt A, pt B, pt C) {return  angle(V(B,A),V(B,C)); }                                       // angle <BA,BC>
float turnAngle2(pt A, pt B, pt C) {return  angle(V(A,B),V(B,C)); }                                   // angle <AB,BC> (positive when right turn as seen on screen)
float angle(vec U, vec V, vec W) {float a = atan2(d(V,U(N(W,U))),d(V,U(U))); if(a<0) a+=TWO_PI; return a; };   // angle(U,V) around W in (0,PI)
...
//// Rotate Q around a vector axis
//pt R(pt Q, float a, vec AXIS) {
//   PVector v = new PVector(Q.x, Q.y, Q.z);
//   PVector _axis = new PVector(AXIS.x, AXIS.y, AXIS.z);
//   PVector nv = Rotate(v, _axis, a);
   
//   return P(nv.x, nv.y, nv.z);
//}
